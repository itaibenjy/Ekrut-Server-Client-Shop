 ...
 *This class is used to handle the RegisteredCustomer's GUI screen.
 * It allows the registered customer to make an order, view catalog, manage his orders and pick up an order.
 * It handles the actions performed by the registered customer.
 *The class has several methods that handle the different button presses on the RegisteredCustomer screen, such as the logOutPressed,
...
public class RegisteredCustomerController  {
	...
	@FXML
	private Button wantToBeSubscriber;
 ...
 // user details
	 ...
	 *and sets the back button as visible if the user's role is not RegisteredCustomer.
	 */
	@FXML
	public void initialize() {
		...
		if(!(userController.getUser().getRole().equals(Role.RegisteredCustomer)))
	...
	/**
	*This method handles the action event when the back button is pressed.
	*It calls the backOrNextPressed(ActionEvent, String) method
	*with the event and the appropriate screen according to the current user's role.
	*@param event the event that triggered this method, in this case the back button being pressed.
	*/
	@FXML
	...
	*This method is triggered when the user presses the 'X' button.
	*It calls the xPressed method from the CommonActionsController class.
	...
	*It calls the logOutPressed method in the CommonActionsController class.
	...
	*/
	...
	/**
	*This method is used to handle the event when the user clicks on the 'Manage My Order' button.
	*It navigates the user to the 'ManageMyOrderScreen.fxml' screen.
	*'ManageMyOrderScreen.fxml' screen the user can view his order history and confirm receipt of deliveries.
	...
	* It navigates the user to the "PickUpOrderScreen.fxml" screen, which allows the user to pick up their order.
	...
	*/
	...
	/**
	*The method handles the event of pressing the view catalog button.
	* It calls the backOrNextPressed(ActionEvent, String)} method from the CommonActionsController class, 
	* the user will be transferred to the view catalog screen to see the all products.
	...
	*/
	...
	/**
	*This method is executed when the "Make an Order" button is pressed. 
	*It checks the current configuration of the client console and either navigates to
	* the Remote Order Screen for remote order(OLbconfiguration)
	*or the My Order Screen for local order(EK configuration).
	...
	*/
	...
	/**
	 * The wantToBeSubscriberPressed method is called when the wantToBeSubscriberBtn is pressed.
	 * It opens a pop-up window with the title "Subscriber registration request",
	 ...
	 */
	@FXML
	void wantToBeSubscriberPressed(ActionEvent event) {

		commonActionsController.popUp(new Stage(),
				"Subscriber registeration request",
				"To register as a subscriber,\n please contact a service representative at the number:\n 0525381648",null,null,null);
